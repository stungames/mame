...
NMI:
	...
	if (UNEXPECTED(m_icount <= 0)) { push_state(35); return; }
state_35:
	dummy_read_opcode_arg(0);
	if (UNEXPECTED(m_icount <= 0)) { push_state(36); return; }
state_36:
	dummy_vma(1);
	if (UNEXPECTED(m_icount <= 0)) { push_state(37); return; }
state_37:
	m_cc |= CC_E;
	set_regop16(m_s);
...
FIRQ:
	dummy_read_opcode_arg(0);
	if (UNEXPECTED(m_icount <= 0)) { push_state(39); return; }
state_39:
	dummy_read_opcode_arg(0);
	if (UNEXPECTED(m_icount <= 0)) { push_state(40); return; }
state_40:
	dummy_vma(1);
...
state_138:
	set_regop16(m_s);
	m_cc = read_memory(regop16().w++); 	if (UNEXPECTED(m_icount <= 0)) { push_state(139); return; }
state_139:
// PULS CC
